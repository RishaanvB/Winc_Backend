...
def best_average_rating() -> models.Restaurant:
    ...
    """
    best = (
        models.Restaurant.select(models.Restaurant, peewee.fn.AVG(models.Rating.rating))
        .join(models.Rating)
...
def add_rating_to_restaurant() -> None:
    ...
    """
    rest = models.Restaurant.get(1)
    models.Rating.create(restaurant=rest, rating=5, comment="zeer lekker eten")
...
def add_dish_to_menu() -> models.Dish:
    ...
    cheese_sandwich = models.Dish.create(
        served_at=restaurant, price_in_cents=128000, name="Cheese Sandwich"
    )
    cheese_sandwich.ingredients.add([cheese, bread])
    return cheese_sandwich
